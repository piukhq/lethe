...
RUN addgroup --gid 1550 apps && \
 ...
 apt-get update && \
 apt-get -y install rsync git git-core python3 python3-pip python3-dev nginx curl && \
 ...
 rsync -a --remove-source-files /usr/local/src/lethe/docker_root/ / && \
 pip3 install --upgrade pip && \
 pip3 install uwsgi && \
 pip3 install -r /usr/local/src/lethe/requirements.txt && \
